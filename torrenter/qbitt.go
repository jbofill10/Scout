...
func NewQbittHandler(qCfg *models.QbittCfg, pCfg *models.ProwlarrCfg, repo *Repo, logger *log.Logger) (*QbittHandler, error) {
...
func (q *QbittHandler) calcIndexerIDs(mediaType string, isAnime bool) []int64 {
	if isAnime {
...
func (q *QbittHandler) isCorrectTorrent(torrent *prowlarr.Search, strategy *models.SearchStrategy) bool {
	...
	foundEpisode := false
	if slices.Contains(strings.Fields(torrent.SortTitle), standardizeNumber(strategy.Episode)) {
		foundEpisode = true
	}
...
func (q *QbittHandler) pickBestTorrent(matches []*models.TorrentMatch, mediaType string, isAnime bool) *models.TorrentMatch {
	...
	rows, err := q.repo.db.Query("SELECT uploaderName FROM UploaderPreferences WHERE mediaType = $1 AND isAnime = $2", mediaType, isAnime)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&uploader); err != nil {
			q.logger.Printf("Error scanning uploader: %v", err)
			continue
		}
