...
func (q *QbittHandler) downloadTorrent(torrent *prowlarr.Search) error {
	...
	// Diagnostic: Try decoding the GUID to see if Prowlarr encoded it
	decodedGUID, err := url.QueryUnescape(torrent.GUID)
	if err != nil {
		q.logger.Warn("Failed to decode GUID", "error", err, "guid", torrent.GUID)
		decodedGUID = torrent.GUID // Use original if decode fails
	}

	// Log both versions for comparison
	q.logger.Info("Magnet link analysis",
		"original_guid", torrent.GUID,
		"decoded_guid", decodedGUID,
		"are_different", torrent.GUID != decodedGUID)
