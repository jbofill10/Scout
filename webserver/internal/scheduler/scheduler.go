...
func NewScheduler(repo SchedulerRepository, logger *log.Logger) *Scheduler {
	...
	go func() {
		...
		for _, media := range mediaList {
			...
			if len(media.Metadata.Episodes) > 0 {
				...
				if err != nil {
					s.logger.Printf("failed to parse aired date: %v", err)
					continue
				}
				...
				go func(m tvdb.Media, rt time.Time) {
					...
					<-timer.C
					s.logger.Printf("Queueing media: %s", m.Name)
					queue <- m
				}(media, releaseTime)
			}
		}
