...
func (s *SchedulerRepoTestSuite) TestMarkQueued_Success() {
	s.mock.ExpectExec(`UPDATE ScheduledEpisodes SET schedule_status`).
		WithArgs(1).
		WillReturnResult(sqlmock.NewResult(0, 1))

	err := s.repo.MarkQueued(1)

	s.NoError(err)
	s.NoError(s.mock.ExpectationsWereMet())
}

func (s *SchedulerRepoTestSuite) TestMarkQueued_DatabaseError() {
	s.mock.ExpectExec(`UPDATE ScheduledEpisodes SET schedule_status`).
		WithArgs(1).
		WillReturnError(sql.ErrConnDone)
